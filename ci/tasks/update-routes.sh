...
export MAIN_ROUTE_HOSTNAME=main-$CF_APP_PREFIX

export NEXT_APP_COLOR=$(cat ./current-app-info/next-app.txt)
export NEXT_APP_HOSTNAME=$CF_APP_PREFIX-$NEXT_APP_COLOR

export CURRENT_APP_COLOR=$(cat ./current-app-info/current-app.txt)
export CURRENT_APP_HOSTNAME=$CF_APP_PREFIX-$CURRENT_APP_COLOR
